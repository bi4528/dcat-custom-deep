...
EXAMPLES_CSV = config["paths"]["opsi_column_examples_csv"]
...
def create_prompt(examples, current_col, sample_values):
    ...
    if examples:
        ...
        prompt += f"Given the following examples of columns:\n{examples_text}"
...
# -------------------- Main --------------------
def main():
    logging.info("Loading FAISS, ID tracker, and example metadata...")
    index = faiss.read_index(FAISS_INDEX_PATH)
    with open(ID_TRACKER_JSON, "r", encoding="utf-8") as f:
        id_tracker = json.load(f)
    df_examples = pd.read_csv(EXAMPLES_CSV).set_index("column")
    ...
    for file in os.listdir(DATASET_FOR_TEST):
        ...
        for col in df.columns:
            ...
            for idx in indices[0]:
                if idx == -1:
                    continue
                try:
                    dataset_col = id_tracker[idx]  # e.g., opsi2024::DatumPN
                    _, example_col = dataset_col.split("::")
                    row = df_examples.loc[example_col]
                    examples.append({
                        "column": example_col,
                    ...
                    logging.warning(f"Failed to retrieve example for idx {idx}: {e}")
        ...
        logging.info(f"Running LLM generation for {len(prompts)} columns...")
